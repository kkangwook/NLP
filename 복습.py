...
-3. 결측치 있는 행/열 각각 지우는 척 하기
        -> SW의 ' '를 nan으로 치환 후 결측치 확인 후 java로 채우기-> 워래 결측치에는 뭐가 들어가야할까? (0,평균,최빈값, median)
...
-5. 존재하는 데이터타입은?->current_time이라는 현재시간 새로운 열 만들고 타입확인하고 문자열로 변환 후 시간으로 바꾸고 제거
        학년을 카테고리 타입으로->카테고리 지정(없는 값을 카테고리로 하게되면?) -> 3,1,2순서 지정->정렬하기     # 정답: df[col]=df[col].astype('category')-> df[col]=df[col].cat.set_categories([3,1,2],ordered=True)
...
-7. 이상치처리 함수 만들고 키 기준 2이상인 애들 제거하는 척 하기/ 박스플롯으로 IQR로도 확인해보기
        ...
        !!! 박스플롯 이상치 제거하고 또 boxplot하면 새로운 이상치 생기므로 한번만하고 끝냄!!!
-8. 라벨인코딩을 할 필요가 있는 열은?-> 그 열을 라벨인코딩or 
        one-hot encoding은 가변수 기준화하여 k-1개로 인코딩 후 새로운 열로 추가     by: drop_first=True ->다중공선성 문제 해결 
        범주형 데이터 인코딩 순서 다르게 하는방법은?                     # 카테고리 변화메서드에서 order바꾸면 됨
-9. '국어','영어','수학','과학','사회' 열 정규화 세가지 방법
        이중 로그스케일링은 어떤 데이터에 적합할까? -> 값의 차이가 클때(ex 단백질,유전자 발현량)
        어떤 열은 ss, 어떤 열은 np.log1p(data)하면됨-> data는 1열이며 여러 컬럼에 넣고싶을때는 df[cols].apply(np.log1p) 
-10. 키로 구간화하고 구간별 개수보기
-11. 국어-영어-수학-과학을 x로 사회를y로하고 0.3의 비율로 train-test split
##추가문제##
-12. 키 185이상인 학생의 이름,수학,과학 정보 가져오기(.loc을 사용해서)
-13. 학교데이터에 '등학교'붙이기 -> 키 자료 끝에 'cm'붙이기 -> df int타입에서 각 행/열 별로 sum집계함수+100해주는 함수 적용시키기
-14. 국어+영어+수학+과학+사회를 다 더한 '총합' 이란 새로운 컬럼 생성 -> '결과' 란 새로운 컬럼 만들고 총합이 400 이상이면 결과열을 pass로, 나머지는 fail
-15. SW특기 컬럼이름을 sw로 바꾸기
-16. 학교에 따른 국어,수학,영어 평균값은? -> 학교로 그룹화 한 다음 이중 각 학년 별 학생 수는?->비율로 나타내기->여기서 북산고 애들만 꺼내오기
-17. 밑의 데이터를 concat, merger두가지 방식으로 합쳐보기
a={'a':[1,2,3],'b':[4,5,6]}
b={'a':[1,3,9],'c':[10,11,12]}
adf=pd.DataFrame(a)
bdf=pd.DataFrame(b)
